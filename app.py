...
# ==========================================
    ...
    # --- MODULE 4: VOICE ASSISTANT ---
        ...
        if st.button("🔊 " + ("ऑडियो उत्तर चलाएं" if is_hi else "Play Audio Response"), type="primary", use_container_width=True):
            ...
            # Using gTTS for reliable native audio generation & playback
            try:
                lang_code = 'hi' if is_hi else 'en'
                tts = gTTS(text=ans_text, lang=lang_code)
                ...
                st.error("⚠️ Audio generation error. Please ensure `gTTS` is added to your requirements.txt.")
